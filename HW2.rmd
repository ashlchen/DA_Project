...
library(glmnet)
```
# Download "purchase.csv" data file from Canvas. Make sure you read the description of Question 1 (in HW2 file) before doing this part. Import the data into R
...
# 1. Use 5-fold cross-validation to evaluate the performance of the regression tree model on this dataset, report the average (1) MAE; (2) RMSE. Then, build a single regression tree model on the entire dataset, plot the tree, and answer the following two questions.
# (a) How many decision nodes are there in your tree?
# (b) Pick one decision rule from your tree and interpret it
...
rmses_tree = c() #initializing for CV
maes_tree = c()
